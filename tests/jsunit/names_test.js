...
function test_safeName() {
  ...
  assertEquals('SafeName number start.', 'my_9lives', varDB.safeName_('9lives'))
...
function test_getDistinctName() {
  var varDB = new Blockly.Names('window,door')
  assertEquals('Name distinct #1.', 'Foo_bar', varDB.getDistinctName('Foo.bar', 'var'))
  assertEquals('Name distinct #2.', 'Foo_bar2', varDB.getDistinctName('Foo.bar', 'var'))
  assertEquals('Name distinct #3.', 'Foo_bar3', varDB.getDistinctName('Foo.bar', 'proc'))
  varDB.reset()
  assertEquals('Name distinct #4.', 'Foo_bar', varDB.getDistinctName('Foo.bar', 'var'))
}
